...
go run github.com/randall77/makefat@v0.0.0-20260406194835-1b91746796b7 \
	"$OUT/bundle/server/doc-scraper-macos" \
	"$(find_bin darwin_amd64 doc-scraper)" \
	"$(find_bin darwin_arm64 doc-scraper)"
cp "$(find_bin linux_amd64 doc-scraper)" "$OUT/bundle/server/doc-scraper-linux"
cp "$(find_bin windows_amd64 doc-scraper.exe)" "$OUT/bundle/server/doc-scraper.exe"
